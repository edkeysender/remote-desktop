...
The brand mark is `assets/hangar.ico` (generated by `tools/gen-icon.ps1` — gradient rounded
square + the Hangar roof glyph, multi-size BMP frames); it's the exe/installer icon for all
...
`HANGAR_BRAND_*` env vars). This must run on a Windows build box — the Linux/Pi relay cannot
