...
- Host a **multi-tenant account web app** (Express, `server/lib/*` + `server/public/`)
  ...
  computer iff same org AND (admin OR shares a group with it).
    - A **host claims a computer by signing in**: `register` carries `{token(device),
      ...
      computer list.
    - **Or enrolls via a pre-baked token** (AllViewer plan Phase 0): an admin generates an
      ...
      The dashboard (AllViewer-branded: device grid, groups, users, enrollment) shows it
      online and it's controllable through the relay by org members. This satisfies the
      Phase 0 exit criteria in `claude/allviewer-development-plan-with-html.md` §5 on the
...
The brand mark is `assets/allviewer.ico` (generated by `tools/gen-icon.ps1` — gradient rounded
square + the AllViewer roof glyph, multi-size BMP frames); it's the exe/installer icon for all
...
`ALLVIEWER_BRAND_*` env vars). This must run on a Windows build box — the Linux/Pi relay cannot
...
- **Phase 4 — product (IN PROGRESS):** host as a **Windows Service** (survives logout,
  ...
  `BackgroundService` (`Supervisor`) that keeps one capture worker — `AllViewerAgent.exe
  ...
  `%ProgramData%\AllViewer`, self-provisions a stable `HostToken` + fixed password on
  ...
  `%ProgramData%\AllViewer\machine.json`; the ID field polls until the worker connects).
...
- **In-app auto-update:** both apps check `<http>/update/manifest.json` on launch and,
  ...
  **client** elevates (Program Files + it stops/starts the SYSTEM service so the worker
  releases `AllViewerAgent.exe`); the **master** installs per-user and updates without
